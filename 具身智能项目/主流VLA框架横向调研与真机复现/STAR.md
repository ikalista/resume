...
**任务**：掌握 LeRobot 框架并跑通各项入门任务；对 ACT、OpenVLA、π 系列等主流方案进行架构拆解与横向对比；完成阶段性仿真复现。

**行动**：
- 系统拆解 ACT、OpenVLA、π0/π0.5/π0.6/π-fast 等框架，归纳三类技术路线（动作离散化 / 扩散与 Flow Matching / 世界模型联合优化），梳理各主流数据集在语言泛化、双臂协同与真机迁移中的适用边界
